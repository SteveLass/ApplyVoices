...
def main():
    ...
    with st.sidebar:
        ...
        recording_duration = st.slider("Max Recording Duration (seconds)", 5, 300, 30)
                ...
                try:
                    with st.spinner(f"Processing and transcribing with {selected_api_name}..."):
                        ...
                        result = sr_manager.transcribe_audio_file(
                            tmp_file_path, api_name, language_code, api_key, asr_pipeline
                        )
    ...
    with col2:
        ...
        <div class="feature-box">
            ...
            <p>Record and transcribe speech in real-time with pause/resume</p>
        </div>
        """, unsafe_allow_html=True)
                        ...
                        with tempfile.NamedTemporaryFile(delete=False, suffix='.wav') as tmp_file:
                            ...
                            if not st.session_state.recording:
                                with st.spinner(f"Transcribing with {selected_api_name}..."):
                                    result = sr_manager.transcribe_audio_file(
                                        tmp_file.name, api_name, language_code, api_key, asr_pipeline
                                    )
